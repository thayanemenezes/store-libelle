...
export default function SelecaoFormaPagamento(
    ...
    function renderizarItem(label: string, tipo: FormaPagamento) {
        ...
        return (
            <button
                className="flex items-center gap-3 text-pink-900 rounded-lg h-12 px-7"
                onClick={() => props.formaPagamentoMudou?.(tipo)}
            >
                <span
                    className={`
                        ${selecionado ? 'bg-pink-600 border-pink-300' : 'bg-gray-500 border-white'}
    ...
    return (
        <div className={`flex flex-col gap-3 ${props.className ?? ''}`}>
            <span className="px-7 pb-2 text-xl font-bold text-pink-950/80">
                Forma de Pagamento
            </span>
